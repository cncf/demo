...
#    availability-id = "${ azurerm_availability_set.cncf.id }"
   ...
   k8s-apiserver-key = "${file("${ var.data-dir }/.cfssl/k8s-apiserver-key.pem")}"
   k8s-apiserver = "${file("${ var.data-dir }/.cfssl/k8s-apiserver.pem")}"
   k8s-etcd-key = "${file("${ var.data-dir }/.cfssl/k8s-etcd-key.pem")}"
   k8s-etcd = "${file("${ var.data-dir }/.cfssl/k8s-etcd.pem")}"
   ca = "${file("${ var.data-dir }/.cfssl/ca.pem")}"
...
module "worker" {
  ...
  internal-tld = "${ var.internal-tld }"
  ca = "${file("${ var.data-dir }/.cfssl/ca.pem")}"
  k8s-worker = "${file("${ var.data-dir }/.cfssl/k8s-worker.pem")}"
  k8s-worker-key = "${file("${ var.data-dir }/.cfssl/k8s-worker-key.pem")}"
  # cloud-config = "${file("${ var.data-dir }/azure-config.json")}"
